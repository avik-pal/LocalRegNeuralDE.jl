...
function main(config_name::String, cfg::ExperimentConfig)
  ...
  loggers = create_logger(log_dir, cfg.train.total_steps - initial_step,
                          ...
                          flatten_configuration(cfg); sde=true)
  ...
  for step in initial_step:(cfg.train.total_steps)
    ...
    loggers.progress_loggers.train.avg_meters.nfe_drift(stats.nfe[1], bsize)
    loggers.progress_loggers.train.avg_meters.nfe_diffusion(stats.nfe[2], bsize)
    ...
    if step % cfg.train.evaluate_every == 1 || step == cfg.train.total_steps
      ...
      for (x, y) in ds_test
        ...
        loggers.progress_loggers.eval.avg_meters.nfe_drift(stats.nfe[1], bsize)
        loggers.progress_loggers.eval.avg_meters.nfe_diffusion(stats.nfe[2], bsize)
